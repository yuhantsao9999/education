        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < obj.data.Course_detail.length; i++) {
                ...
                for (j = 0; j < obj.data.Course_detail[i].Chapter_detail.length; j++) {
                    ...
                    videoNowVideo.src =
                        'https://d3u7d6vbm9yroa.cloudfront.net/' + obj.data.Course_detail[i].Chapter_detail[j].Video;
...
function course_update_submit() {
    document.getElementById('submit').addEventListener('click', function () {
        ...
        for (var i = 0; i < chapter_num; i++) {
            ...
            each_chapter_section_num_arr.push(
                document.getElementsByName('chapter_title')[i].parentNode.parentNode.parentNode.childNodes.length - 1
            );
        }
