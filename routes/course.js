...
router.post("/course/teacher/info", function(req, res) {
    ...
    var mysql_course_info = `SELECT new_course.course_teacher,user.about_me,user.PersonalWebsite,user.facebookProfile,user.youtubeProfile,user.user_image
    FROM new_course Join user ON new_course.course_teacher=user.name where course_title='${course_title}';`
    con.query(mysql_course_info, function(err, result_course_info) {
        if (err) throw err;
